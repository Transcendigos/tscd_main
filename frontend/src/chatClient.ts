...
interface User {
  id: number; // Assuming this will be the raw numeric ID from the server's user list API
  username: string;
  picture?: string | null;
}
...
const activePrivateChats = new Map<number, ActiveChatWindow>(); // Keyed by numeric peer ID

interface ChatMessage {
  id?: number;
  type?: string; // Allow for Pong message types too
  fromUserId?: number | string; // Can be numeric (self) or prefixed string (from server)
  fromUsername?: string;
  toUserId?: number | string; // Can be numeric (self) or prefixed string (from server)
  content?: string; // Optional for non-chat messages
  timestamp?: string;
  message?: string; // For error messages
  user?:
    | User
    | { userId: number | string; username: string; picture?: string | null }; // For auth_success or userOnline/Offline
  messageId?: number;
  gameId?: string;
...
let currentUserId: number | null = null; // Store as raw numeric ID
...
export function initializeChatSystem() {
  ...
  if (!chatUserListEl) {
    console.error(
      "Main Chat User List element not found! Cannot initialize chat system."
    );
    return;
  }
  connectWebSocket();
  ...
  chatUserListEl.addEventListener("click", (event) => {
    ...
    if (
      listItem &&
      listItem.parentElement === chatUserListEl &&
      listItem.dataset.userId
    ) {
      const userIdNumeric = parseInt(listItem.dataset.userId, 10); // Assume dataset.userId is numeric
...
export function sendPongPlayerInput(
  gameId: string,
  input: "up" | "down" | "stop_up" | "stop_down"
) {
...
async function handleInvitePlayerToPong(opponent: User) {
  // opponent.id is numeric
  if (!opponent || typeof opponent.id === "undefined") {
    console.error("Invalid opponent data for Pong invite.");
    alert("Cannot invite player: invalid opponent data.");
    return;
  }
  const opponentPlayerIdForAPI = `user_${opponent.id}`; // Backend expects "user_X"

  console.log(
    `[Frontend] handleInvitePlayerToPong called for opponent:`,
    opponent,
    `API ID: ${opponentPlayerIdForAPI}`
  );
  try {
    ...
    if (response.ok) {
      ...
      if (gameId) {
        console.log(
          `[Frontend] Game created with ID: ${gameId}. Inviter sending PONG_JOIN_GAME.`
        );
        alert(
          `Pong invitation sent to ${opponent.username}! Starting game room...`
        );
        sendPongJoinGame(gameId);
      } else {
        console.error("Game created, but no gameId in response:", responseData);
        alert(
          `Error starting game: ${
            responseData.message || "No game ID returned from server."
          }`
        );
      }
    } else {
      console.error("Failed to create game API error:", responseData);
      alert(
        `Error inviting to game: ${
          responseData.error || response.statusText || "Unknown server error"
        }`
      );
    }
  } catch (error) {
    console.error(
      "[Frontend] Network or unexpected error inviting player to game:",
      error
    );
    alert(
      "Could not contact server to invite player to game. Please try again."
    );
  }
}
...
function connectWebSocket() {
  ...
  socket.onopen = () => {
    console.log(
      "Chat: WebSocket Connection Opened! Waiting for server to authenticate via cookie."
    );
  };
  ...
  socket.onmessage = (event) => {
    try {
      ...
      if (message.type === "auth_success" && message.user) {
        const userIdFromServer = message.user.userId; // This might be "user_X" or X
        if (
          typeof userIdFromServer === "string" &&
          userIdFromServer.startsWith("user_")
        ) {
          ...
          console.error(
            "Chat: Unparseable userId in auth_success:",
            userIdFromServer
          );
          return;
        }
        ...
        console.log(
          `Chat: Authenticated successfully as ${currentUsername} (Numeric ID: ${currentUserId})`
        );
        if (currentUserId) loadUserList();
        else console.error("Chat: currentUserId not set after auth_success.");
      } else if (
        message.type === "userOnline" ||
        message.type === "userOffline"
      ) {
        console.log(
          `Chat: User ${message.type}, reloading user list:`,
          message.user
        );
        loadUserList();
        ...
        if (
          ...
          message.fromUserId.startsWith("user_")
        ) {
        ...
        if (
          ...
          message.toUserId.startsWith("user_")
        ) {
        ...
        if (numericSenderId === null || isNaN(numericSenderId)) {
          console.warn(
            "Chat: Received newMessage with unparseable fromUserId.",
            message
          );
          return;
        }
        ...
        if (numericSenderId === currentUserId) {
          ...
          console.warn(
            "Chat: Received newMessage not directly involving current user.",
            message,
            currentUserId
          );
          return;
        ...
        if (
          numericPeerIdForMapLookup === null ||
          isNaN(numericPeerIdForMapLookup)
        ) {
          console.warn(
            "Chat: Could not determine relevant numeric peer ID for newMessage.",
            message
          );
          return;
        }
        ...
        if (chatInfo) {
          ...
          const fromUsername =
            message.fromUsername || `User ${numericSenderId}`;
          console.log(
            `Chat: Rcvd message for numeric peer ID ${numericPeerIdForMapLookup} (original sender: ${message.fromUserId}), no window open. User: ${fromUsername}`,
            message
          );
          if (numericSenderId !== currentUserId) {
            alert(
              `New message from ${fromUsername}! Open their chat from the user list to see it.`
            );
          }
        }
        ...
        if (gameId && inviterUsername) {
          console.log(
            `Received Pong game invite for game ${gameId} from ${inviterUsername} (ID: ${inviterId})`
          );
          if (
            confirm(
              `${inviterUsername} has invited you to a game of Pong! Accept?`
            )
          ) {
            console.log(`Accepting Pong invite for game: ${gameId}`);
            sendPongJoinGame(gameId);
          } else {
            console.log(`Declined Pong invite for game: ${gameId}`);
          }
        } else {
          console.warn("Received PONG_GAME_INVITE with missing data:", message);
        }
        ...
        console.log(
          `PONG_GAME_STARTED received for game ${gameId}. You are ${yourPlayerId}. Opponent: ${opponentUsername} (${opponentId})`,
          initialState
        );
        const gameStartEvent = new CustomEvent("pongGameStart", {
          detail: {
            gameId,
            initialState,
            yourPlayerId,
        ...
        const gameUpdateEvent = new CustomEvent("pongGameStateUpdate", {
          detail: message,
        });
        ...
        const gameOverEvent = new CustomEvent("pongGameOver", {
          detail: { gameId, winnerId, scores },
        });
        window.dispatchEvent(gameOverEvent);
      } else if (message.type === "PONG_ERROR") {
        const { gameId, message: pongErrorMessage } = message;
        console.error(
          `Pong Error for game ${gameId || "N/A"}: ${pongErrorMessage}`
        );
        ...
        if (
          ...
          (message.message.toLowerCase().includes("authentication required") ||
            message.message.toLowerCase().includes("authentication failed"))
        ) {
          alert(
            "Chat session could not be established or was terminated. Please ensure you are logged in."
          );
        }
      } else {
        console.warn(
      ...
      console.error(
        "Chat: Error processing message from server",
        e,
        event.data
      );
    }
  ...
  socket.onclose = (event) => {
    ...
    activePrivateChats.forEach((chatInfo) => {
      ...
      const headerSpan = chatInfo.windowInstance.element?.querySelector(
        `#privateChatDragHandle_${chatInfo.peer.id} span .font-bold`
      );
      if (headerSpan)
        (
          headerSpan as HTMLElement
        ).textContent = `Chat with ${chatInfo.peer.username} (Disconnected)`;
    });
    if (chatUserListEl)
      chatUserListEl.innerHTML =
        '<li class="text-slate-400 text-xs p-1.5">Chat disconnected. Please refresh or log in.</li>';
...
async function loadUserList() {
  ...
  if (!currentUserId) {
    console.warn(
      "Chat: Not authenticated (currentUserId not set). Cannot load user list."
    );
    chatUserListEl.innerHTML =
      '<li class="text-slate-400 text-xs p-1.5">Authentication pending...</li>';
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401)
        alert(
          "Chat: Session expired or unauthorized for user list. Please log in again."
        );
      return;
    }
    const usersFromServer: Array<{
      id: number | string;
      username: string;
      picture?: string | null;
    }> = await response.json();
    ...
    if (usersFromServer.length === 0) {
      chatUserListEl.innerHTML =
        '<li class=" text-xs p-1.5">No other users available.</li>';
    } else {
      usersFromServer.forEach((user) => {
        ...
        if (typeof user.id === "string" && user.id.startsWith("user_")) {
          ...
          console.warn(
            "Skipping user with unparseable ID from /api/chat/users:",
            user
          );
          return;
        }
        ...
        li.dataset.userId = numericUserId.toString(); // Store numeric ID in dataset
        if (user.picture) li.dataset.userPicture = user.picture;
        li.className =
          "p-1.5 hover:bg-slate-700 cursor-pointer text-xs flex items-center space-x-2";
    ...
    if (chatUserListEl)
      chatUserListEl.innerHTML =
        '<li class="text-red-400 text-xs p-1.5">Failed to load users.</li>';
  }
}

function createPrivateChatWindowHtml(peerUser: User): string {
  // peerUser.id is numeric
  const peerIdNumeric = peerUser.id;
  const peerUsernameSafe = (peerUser.username || "User").replace(
    /[&<>"']/g,
    (match) =>
      ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[
        match
...
async function launchPrivateChatWindow(peerUser: User) {
  ...
  requestAnimationFrame(async () => {
    ...
    if (!newWindowElement) {
      console.error(
        `Failed to find new window element ${windowId} after insertion.`
      );
      return;
    }
    ...
    try {
      ...
      const invitePongButton = document.getElementById(
        invitePongButtonId
      ) as HTMLButtonElement;

      if (!messagesArea || !inputField || !sendButton || !invitePongButton) {
        console.error(
          `Chat: Could not find all interactive chat sub-elements for ${peerUser.username}.`
        );
      ...
      invitePongButton.addEventListener("click", () => {
        invitePongButton.disabled = true;
        invitePongButton.textContent = "Invited";
        ...
        setTimeout(() => {
          ...
          if (currentButton) {
            ...
            currentButton.textContent = "Invite Pong";
          }
        }, 5000);
      });
      ...
      console.error(
        `Error initializing DesktopWindow or chat for ${peerUser.username}:`,
        error
      );
...
async function loadChatHistoryForWindow(
  peerUser: User,
  messagesArea: HTMLElement
) {
  // peerUser.id is numeric
  if (!currentUserId) {
    ...
    messagesArea.innerHTML =
      '<div class="text-center text-xs text-red-400 p-2">Authentication error.</div>';
    return;
  }
  messagesArea.innerHTML =
    '<div class="text-center text-xs text-slate-400 p-2">Loading history...</div>';
  try {
    const response = await fetch(`/api/chat/history/${peerUser.id}`, {
      method: "GET",
      credentials: "include",
    }); // Use numeric peerUser.id
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        `Chat: Failed to load chat history for ${peerUser.username}`,
        response.status,
        errorText
      );
    ...
    const rawHistory: ChatMessage[] = await response.json(); // Assume ChatMessage is compatible
    messagesArea.innerHTML = "";
    if (rawHistory.length === 0) {
      messagesArea.innerHTML =
        '<div class="text-center text-xs text-slate-400 p-2">No messages yet. Start the conversation!</div>';
    } else {
    ...
    console.error(
      `Chat: Exception loading chat history for ${peerUser.username}:`,
      error
    );
...
function sendMessageToPeer(peerUser: User, content: string) {
  ...
  const messageToSendPayload = {
    type: "privateMessage",
    toUserId: peerUser.id.toString(),
    content: content,
  }; // Send numeric string ID
  socket.send(JSON.stringify(messageToSendPayload));
  console.log(
    "Chat: Sent private message payload to peer:",
    peerUser.id,
    messageToSendPayload
  );

  const chatInfo = activePrivateChats.get(peerUser.id); // Look up with numeric peerUser.id
  if (chatInfo) {
    displayMessageInWindow(
      {
        fromUserId: currentUserId as number,
...
function displayMessageInWindow(
  msg: ChatMessage,
  messagesArea: HTMLElement,
  peerIdOfThisWindowNumeric: number
) {
  let msgFromNumericId: number;
  if (
    typeof msg.fromUserId === "string" &&
    msg.fromUserId.startsWith("user_")
  ) {
    ...
    console.warn(
      "[DisplayMessage] Unparseable msg.fromUserId:",
      msg.fromUserId,
      msg
    );
    return;
  ...
  if (isNaN(msgFromNumericId)) {
    console.warn(
      "[DisplayMessage] msg.fromUserId resulted in NaN:",
      msg.fromUserId,
      msg
    );
    return;
  ...
  console.log(
    `[DisplayMessage] msgFromNumericId: ${msgFromNumericId}, currentUserId: ${currentUserId}, peerIdOfThisWindowNumeric: ${peerIdOfThisWindowNumeric}, fromUsername: ${msg.fromUsername}`
  );

  const messageContainerDiv = document.createElement("div");
  const senderInfoDiv = document.createElement("div");
  const messageContentDiv = document.createElement("div");
  senderInfoDiv.textContent = `${
    msg.fromUsername || `User ${msgFromNumericId}`
  }:`;
  ...
  if (msgFromNumericId === currentUserId) {
    messageContainerDiv.className =
      "text-[#FFE2BF] flex flex-row items-start my-2 py-2";
    ...
    console.warn(
      "[DisplayMessage] Message is not from self and not from the expected peer of this window.",
      {
        msgFromNumericId,
        currentUserId,
        peerIdOfThisWindowNumeric,
        originalMsg: msg,
      }
    );
    return;
...
export function resetChatSystem() {
  ...
  if (chatUserListEl)
    chatUserListEl.innerHTML =
      '<li class="text-slate-400 text-xs">Logged out.</li>';
